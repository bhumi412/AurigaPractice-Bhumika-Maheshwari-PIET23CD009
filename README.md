...
## 13. Likely interviewer questions

**Q: Why not just `ORDER BY priority, sla_deadline` in SQL?**
Because "overdue" depends on the current time, not a stored value - there's
no column to sort on. Either you store a boolean and update it constantly
(fragile, and wrong the instant it's not updated), or you compute it at read
time, which is what this does.

**Q: Why does overdue outrank priority instead of the reverse?**
A broken promise is worse than a promise you can still keep. It also has a
nice property: because time only moves forward, a ticket can't un-overdue
itself, so the ordering never flaps while the app is open.

**Q: What happens with two tickets that have literally identical
priority, deadline, and creation time?**
The zero-padded ticket ID breaks the tie, deterministically. This is tested
directly - shuffle the same tickets three different ways and the sorted
output is identical every time.

**Q: How do you know overdue detection actually reacts to time passing,
not just to data changes?**
The `now` parameter is threaded through every ranking function instead of
calling `Date.now()` internally. A test creates one ticket object and calls
`isOverdue`/`sortTicketsByUrgency` twice with two different `now` values -
same ticket, different answer.

**Q: Why keep ranking as a separate module instead of doing it in the SQL
query or in a React component?**
Because it's the one piece of logic the whole assignment is testing. Keeping
it a pure function of `(tickets, now)` with zero framework dependencies means
it's reusable (API and, if needed, client), and the test file reads like a
specification rather than an integration test.

**Q: How does pagination stay correct while tickets are being filtered and
searched?**
Filtering happens in SQL first, ranking happens over the *entire* filtered
result, and only then is a page sliced off. There's a dedicated test that
proves this: with 5 tickets and page size 2, page 1 and page 2 together
produce the same order as ranking all 5 at once - pagination never
reshuffles anything.

**Q: What would you change for a much larger, production version of this?**
Push the bucket/priority ordering into a SQL `ORDER BY` expression for
active tickets (accepting some duplication with the TS comparator, which
would become the test oracle for the SQL), add real auth in place of the
hard-coded `CURRENT_USER`, and consider SQLite FTS5 (or Postgres full-text
search) for non-prefix customer search.

**Q: Why SQLite instead of Postgres/Mongo?**
Zero infrastructure for a coding assessment, real SQL and real indexes (so
the filtering story is genuine, not a toy), and a one-line swap to Postgres
later since the repository is the only file that touches SQL directly.

**Q: How is "assigned to me" different from a specific assignee filter?**
Mechanically nothing - both hit the same `assignee=<name>` query param. It's
a UI convenience: the dropdown pre-fills the current (hard-coded) user's name
as one of the options, next to "Unassigned" and "All".

**Q: Why is `firstRespondedAt` in the model but not used in the ranking
rule?**
The SLA promise is technically about *responding*, not resolving - but the
brief's stated rule scopes "active" to ticket status, so that's what's
implemented. The field is there so a future iteration (or a reporting query)
could switch to it without a schema change.
